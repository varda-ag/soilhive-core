...
export const setConfigEntitlement = async (req: Request, res: Response) => {
  const key = req.params['configId']! as string;
  const data = await entitlementService.setConfigEntitlement(req.customData, key, req.body);
  res.json(data);
};
...
export const getUserEntitlements = async (req: Request, res: Response) => {
  ...
  res.json(await entitlementService.getVisibleEntitlements(req.customData, scope));
};
