...
export class RefactorProcedures1774333921813 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "procedures" ADD COLUMN sample_pretreatment_category "vocabulary_category_enum" GENERATED ALWAYS AS ('sample_pretreatment'::"vocabulary_category_enum") STORED, ADD COLUMN laboratory_method_category "vocabulary_category_enum" GENERATED ALWAYS AS ('laboratory_method'::"vocabulary_category_enum") STORED, ADD COLUMN extractant_concentration_category "vocabulary_category_enum" GENERATED ALWAYS AS ('extractant_concentration'::"vocabulary_category_enum") STORED, ADD COLUMN extraction_ratio_category "vocabulary_category_enum" GENERATED ALWAYS AS ('extraction_ratio'::"vocabulary_category_enum") STORED, ADD COLUMN extraction_base_category "vocabulary_category_enum" GENERATED ALWAYS AS ('extraction_base'::"vocabulary_category_enum") STORED, ADD COLUMN measurement_procedure_category "vocabulary_category_enum" GENERATED ALWAYS AS ('measurement_procedure'::"vocabulary_category_enum") STORED, ADD COLUMN limit_of_detection_category "vocabulary_category_enum" GENERATED ALWAYS AS ('limit_of_detection'::"vocabulary_category_enum") STORED`,
    );
    ...
    await queryRunner.query(`INSERT INTO "vocabulary" ("name", "category")
                SELECT * FROM (
                    SELECT DISTINCT sample_pretreatment, 'sample_pretreatment'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT laboratory_method, 'laboratory_method'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT extractant_concentration, 'extractant_concentration'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT extraction_ratio, 'extraction_ratio'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT extraction_base, 'extraction_base'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT measurement_procedure, 'measurement_procedure'::"vocabulary_category_enum" FROM "procedures"
                    UNION ALL
                    SELECT DISTINCT limit_of_detection, 'limit_of_detection'::"vocabulary_category_enum" FROM "procedures") as v("name", "category")
    ...
    await queryRunner.query(`UPDATE "procedures" SET sample_pretreatment_id=pv1.id, laboratory_method_id=pv2.id, extractant_concentration_id=pv3.id, extraction_ratio_id=pv4.id, extraction_base_id=pv5.id, measurement_procedure_id=pv6.id, limit_of_detection_id=pv7.id 
                ...
                left join vocabulary pv1 on pv1.name=proc.sample_pretreatment AND pv1.category='sample_pretreatment'
                left join  vocabulary pv2 on pv2.name=proc.laboratory_method AND pv2.category='laboratory_method'
                left join  vocabulary pv3 on pv3.name=proc.extractant_concentration AND pv3.category='extractant_concentration'
                left join  vocabulary pv4 on pv4.name=proc.extraction_ratio AND pv4.category='extraction_ratio'
                left join  vocabulary pv5 on pv5.name=proc.extraction_base AND pv5.category='extraction_base'
                left join  vocabulary pv6 on pv6.name=proc.measurement_procedure AND pv6.category='measurement_procedure'
                left join  vocabulary pv7 on pv7.name=proc.limit_of_detection AND pv7.category='limit_of_detection'
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`UPDATE "procedures" SET sample_pretreatment=pv1.name, laboratory_method=pv2.name, extractant_concentration=pv3.name, extraction_ratio=pv4.name, extraction_base=pv5.name, measurement_procedure=pv6.name, limit_of_detection=pv7.name 
            FROM procedures proc
            left join vocabulary pv1 on pv1.id=proc.sample_pretreatment_id AND pv1.category='sample_pretreatment'
            left join  vocabulary pv2 on pv2.id=proc.laboratory_method_id AND pv2.category='laboratory_method'
            left join  vocabulary pv3 on pv3.id=proc.extractant_concentration_id AND pv3.category='extractant_concentration'
            left join  vocabulary pv4 on pv4.id=proc.extraction_ratio_id AND pv4.category='extraction_ratio'
            left join  vocabulary pv5 on pv5.id=proc.extraction_base_id AND pv5.category='extraction_base'
            left join  vocabulary pv6 on pv6.id=proc.measurement_procedure_id AND pv6.category='measurement_procedure'
            left join  vocabulary pv7 on pv7.id=proc.limit_of_detection_id AND pv7.category='limit_of_detection'
            where proc.id=procedures.id`);
    await queryRunner.query(
