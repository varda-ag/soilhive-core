...
const JOB_ERROR_MESSAGES: Record<string, JobErrorMessage> = {
  ...
  DR_FILE_NOT_FOUND: {
    ...
    actions: ['Re-upload the file and start the data request job again.'],
  },
  DR_FILE_NOT_SPATIAL: {
    message: 'The file was uploaded as non-spatial, so no aggregation areas can be read from it.',
    actions: ['Re-upload the file as a spatial file, then start the data request job again.'],
  },
  DR_FILE_IS_RASTER: {
  ...
  DR_MISSING_EPSG: {
    ...
    actions: ['Open the file in the admin portal and set its EPSG code, then start the data request job again.'],
  },
  DR_UNKNOWN_LABEL_FIELD: {
  ...
  DR_NO_UNITS: {
    ...
    actions: ['Check the file contains polygon boundaries, then start the data request job again.'],
  },
  DR_TOO_MANY_UNITS: {
  ...
  DR_NO_GEOMETRIES: {
    ...
    actions: ['Draw or upload an area of interest, save the filter, then start the data request job again.'],
  },
