...
export const extractUnitsFromFile = async (
  ...
  const fileEntity = await fileService.getFile(requestData, fileId).catch(error => {
    if (error instanceof ErrorResponse && error.status === StatusCodes.NOT_FOUND) {
      throw new JobError('RUN_FILE_NOT_FOUND', { file_id: fileId });
    }
    throw error;
  });
  ...
  if (!fileEntity.metadata) {
    throw new JobError('RUN_FILE_NOT_SPATIAL', { file_id: fileId });
  }
  if (fileEntity.metadata.is_raster) {
    throw new JobError('RUN_FILE_IS_RASTER', { file_id: fileId });
  }
  const metadata = fileEntity.metadata as VectorFileMetadata;
  if (!metadata.epsg) {
    throw new JobError('RUN_MISSING_EPSG', { file_id: fileId });
  }
  ...
  if (labelField && !metadata.field_names.includes(labelField)) {
    throw new JobError('RUN_UNKNOWN_LABEL_FIELD', { label_field: labelField });
  }
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aggregation-units-'));
  ...
  if (features.length === 0) {
    throw new JobError('RUN_NO_UNITS', { file_id: fileId });
  }
  ...
  if (nonPolygonCount > 0) {
    throw new JobError('RUN_NON_POLYGON_GEOMETRY', { count: nonPolygonCount, file_id: fileId });
  }
...
export const unitsFromFilter = async (requestData: RequestData, geometryIds: string[]): Promise<ExtractedUnits> => {
  if (geometryIds.length === 0) {
    throw new JobError('RUN_NO_GEOMETRIES');
  }
