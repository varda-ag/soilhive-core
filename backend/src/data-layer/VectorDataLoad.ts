...
export default class VectorDataLoad {
  ...
  getDataPreview = async (dataSource: DataSource, fileId: string, dataMappingId: string): Promise<any> => {
    ...
    for (const [field, mapping] of Object.entries(dataMapping)) {
        if (typeof mapping === 'string' || mapping instanceof String) { // metadata fields
          ...
          const propInfo: UnitConversionEntity | null =
              props.conversion_slug !== undefined
                  ? await uc_repo.findOneBy({ slug: props.conversion_slug })
                  : null;
  
          const conversionFormula = propInfo?.conversion_formula 
          ? propInfo.conversion_formula.replace(/"/g, '').trim() 
          : null;
  
          let expr = conversionFormula 
              ? conversionFormula.replace(/x/g, `(raw."${field}")::numeric`) 
              : `(raw."${field}")::numeric`;
    ...
    let query = await dataSource
      .createQueryBuilder()
      .select([
        'raw.record_id as record_id',
        metadataCols.replace(/, $/, ''),
        propertiesCleanup.replace(/, $/, ''),
        'raw.geom',
      ])
