...
export default class SoilPropertyCategoryService {
  getSoilPropertyCategories = async (requestData: RequestData): Promise<SoilPropertyCategory[]> => {
    const repo = requestData.entityManager.getRepository(SoilPropertyCategoryEntity);
    return await repo.find();
  };

  getSoilPropertyCategory = async (requestData: RequestData, slug: string): Promise<SoilPropertyCategory> => {
