...
`convert_raster.sh` is unaffected: converting a GeoTIFF to a COG remains a standalone, prior step, exactly as ADR 0004 decided. (Superseded by ADR 0025: removing the CLI left nobody positioned to run that prior step, so the loader now normalizes the file itself and the script is gone.)
...
- Preconditions the CLI checked loosely are now job failures with actionable messages: a missing band mapping, a band the file does not have, input that is not a COG, and pixel values not already in the property's standard unit. (The last two stopped being failures under ADR 0025 — the loader normalizes both instead.)
