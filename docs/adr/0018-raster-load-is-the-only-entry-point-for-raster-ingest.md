...
`convert_raster.sh` is unaffected: converting a GeoTIFF to a COG remains a standalone, prior step, exactly as ADR 0004 decided.
...
- Preconditions the CLI checked loosely are now job failures with actionable messages: a missing band mapping, a band the file does not have, input that is not a COG, and pixel values not already in the property's standard unit.
