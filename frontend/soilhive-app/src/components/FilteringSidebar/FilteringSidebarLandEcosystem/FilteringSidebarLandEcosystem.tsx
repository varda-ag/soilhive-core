...
export function FilteringSidebarLandEcosystem() {
  const { allRasterCategories, geometryFilterResults, isLoadingRasterCategories } = useAvailability();
  ...
  const allEmpty = !isLoadingRasterCategories && dynamicCategories.length > 0 && dynamicCategories.every(cat => hasNoOptions(cat.id)); // <-- CHANGE: now reads naturally
