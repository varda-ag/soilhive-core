...
type EntityScopedAction = Capability.DOWNLOAD | Capability.PREVIEW | Capability.WRITE;
...
export function useEntitlements(scope?: EntitlementScope) {
  ...
  const can = useCallback(
    (action: Action | EntityScopedAction, entityId?: string): boolean => {
      if (action === Capability.DOWNLOAD || action === Capability.PREVIEW || action === Capability.WRITE) {
        if (!entityId) {
